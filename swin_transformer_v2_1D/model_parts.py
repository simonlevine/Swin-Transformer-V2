...
def bcd_to_bdc(input: torch.Tensor) -> torch.Tensor:
    """
    Permutes a tensor to the shape [batch size, length, channels]
    :param input: (torch.Tensor) Input tensor of the shape [batch size, length, channels]
    :return: (torch.Tensor) Output tensor of the shape [batch size, length, channels]
    """
    return input.permute(0, 2, 1)


def bdc_to_bcd(input: torch.Tensor) -> torch.Tensor:
    """
    Permutes a tensor to the shape [batch size, channels, length]
    :param input: (torch.Tensor) Input tensor of the shape [batch size, length, channels]
    :return: (torch.Tensor) Output tensor of the shape [batch size, channels, length]
    """
    return input.permute(0, 2, 1)
...
def unfold(input: torch.Tensor,
    ...
    :param input: (torch.Tensor) Input feature map of the shape [batch size, channels, length]
    :param window_size: (int) Window size to be applied
    :return: (torch.Tensor) Unfolded tensor of the shape [batch size * windows, channels, window size]
    """
    # Get original shape
    _, channels, length = input.shape  # type: int, int, int
    # Unfold input
    output: torch.Tensor = input.unfold(dimension=2, size=window_size, step=window_size) #-->batch size, channels, windows, window size
